...
def docker_rm_all_with_label(logger, cluster_id=CLUSTER_ID, node_prefix=NODE_PREFIX):
...
def docker_compose_up(logger):
    ...
    run_cmd('docker compose -f ./docker-compose.yml up -d', capture_output=True)
    logger.log_all("Kafka services started.")

def wait_labeled_containers(logger, cluster_id=CLUSTER_ID):
...
def create_topic(logger, bootstrap_server=BOOTSTRAP_SERVER, topic_name=TOPIC_NAME, num_partitions=1, replication_factor=1):
...
def wait_for_topic_ready(logger, bootstrap_server=BOOTSTRAP_SERVER, topic_name=TOPIC_NAME):
    ...
    while True:
        result = run_cmd(f'kafka/bin/kafka-topics.sh --list --bootstrap-server {bootstrap_server}', capture_output=True)
...
def setup_experiment_env(
    ...
    docker_rm_all_with_label(logger, cluster_id, node_prefix)
    ...
    create_topic(logger, bootstrap_server, topic_name, num_partitions, replication_factor)
    wait_for_topic_ready(logger, bootstrap_server, topic_name)
